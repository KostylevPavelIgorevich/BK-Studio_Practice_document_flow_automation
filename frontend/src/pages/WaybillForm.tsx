...
export function WaybillForm({
  ...
  // Инициализация формы с данными из заявки
  useEffect(() => {
  ...
  const validateField = (field: any, value: any): string => {
    ...
    if (field.type === 'date' || field.key.toLowerCase().includes('date')) {
      ...
      if (isNaN(date.getTime())) {
        return `Поле "${field.label}" должно содержать корректную дату`;
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!docType) {
        throw new Error(`Тип документа для формы "${form}" не найден`);
      }
  ...
  return (
    <div className="min-h-screen bg-[#E4E9F8]">
      ...
      <div className="absolute top-4 right-6 z-10 flex gap-3">
        <button onClick={onBack} className="px-4 py-2 bg-[#3ABC96] hover:bg-[#1e4bc2] text-white font-medium rounded-lg shadow-md">
          Назад
        </button>
        ...
        <div className="w-1/2 flex flex-col">
          ...
          <div className="flex gap-4 mt-4">
            <button 
              ...
              className="flex-1 py-3 bg-[#4475F7] hover:bg-[#3662d9] disabled:bg-gray-400 text-white font-medium rounded-lg"
            >
            ...
            <button 
              onClick={handlePrint} 
              disabled={hasErrors} 
              className="flex-1 py-3 bg-[#2860F0] hover:bg-[#1e4bc2] disabled:bg-gray-400 text-white font-medium rounded-lg"
            >
              🖨️ Печать
            </button>
          </div>
